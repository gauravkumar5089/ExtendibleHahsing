...
void split(int hash_value, bucket *splitting_bucket)
{
    bucket *newB = new bucket;
    splitting_bucket->a.clear();

    if (splitting_bucket->local_depth == global_depth)
    {
        ...
        {
            cout << "Directory can't expand now" << endl;
            display();
            return;
        }
    }
    ...
    (splitting_bucket->local_depth)++;
    (storage[new_bucket_number]->local_depth) = splitting_bucket->local_depth;
